...
- Otra opción es crear un entorno con Anaconda que incluya un compilador C++. Puede ser útil si ya usas Anaconda para otros lenguajes como Python y quieres mantener todo en un solo entorno. Se puede instalar el paquete `gxx` para obtener un compilador C++ con el comando `conda install -c conda-forge gxx`.
